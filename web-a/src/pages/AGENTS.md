...
| Models | `llm-models.tsx` | Org + platform admin. PA uses `?company=`. Writes `/api/enterprise/llm-models`. Set primary (`/set-default`), secondary (`/set-secondary`), and fallback (`/set-fallback`). Connect Grok subscription starts/polls device-code (no token fields). |
