...
class SSOUserInfo(TypedDict):
    ...
    mobile: NotRequired[str | None]
    phone: NotRequired[str | None]
    union_id: NotRequired[str]
    unionId: NotRequired[str]
    open_id: NotRequired[str]
    raw_data: NotRequired[JsonObject]


class RegistrationService:
    """Service for handling user registration flows."""

    # ── IdentityRecord provider ────────────────────────────────────────────────────
    ...
    async def handle_sso_registration(
        ...
        if existing_user:
            await sso_service.link_identity(
                ...
                lookup_provider_user_id,
                dict(user_info),
                tenant_id=str(existing_user.tenant_id) if existing_user.tenant_id else sso_tenant_id,
            )
            return existing_user, False

        # Create new IdentityRecord + UserRecord
            ...
            if user_info_obj.email:
                ...
                if existing_by_email:
                    await sso_service.link_identity(
                        ...
                        lookup_provider_user_id,
                        dict(user_info),
                        tenant_id=(str(existing_by_email.tenant_id) if existing_by_email.tenant_id else sso_tenant_id),
                    )
    ...
    async def bind_org_member(self, user: UserRecord) -> None:
        ...
        if member:
            fields: dict[str, Any] = {"user_id": user.id}
            ...
            member = await org_member_dao.update_fields(member.id, fields) or member

            # Keep identity contact in sync when org directory is authoritative.
            identity_updates: dict[str, Any] = {}
            if not user.email and member.email:
                identity_updates["email"] = member.email
                user.email = member.email
            if not user.primary_mobile and member.phone:
    ...
    async def _run_okr_member_hook(self, member_id: uuid.UUID, tenant_id: uuid.UUID | None) -> None:
        ...
        await hook_new_org_member(None, member_id, tenant_id)

    async def ensure_web_org_member(self, user: UserRecord) -> OrgMemberRecord | None:
        ...
        if web_provider.name == "Web":
            web_provider = await identity_provider_dao.update(db_obj=web_provider, obj_in={"name": "Platform"})

        member = await org_member_dao.get_by_user_and_provider(user.id, user.tenant_id, web_provider.id)
        ...
        if member:
            ...
            title = member.title
            if title in (None, "", "Web User"):
                fields["title"] = "Platform User"
            member = await org_member_dao.update_fields(member.id, fields) or member
        else:
    ...
    async def sync_org_member_contact_from_user(
        ...
        *,
        ...
        members = await org_member_dao.get_by_user_and_tenant_and_provider(user.id, user.tenant_id, web_provider.id)
        if not members:
            return
