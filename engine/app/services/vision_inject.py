...
type GridOptions = dict[str, int | float]


class VisionTextContent(TypedDict):
    type: Literal["text"]
    text: str


class VisionImageURL(TypedDict):
    url: str


class VisionImageContent(TypedDict):
    type: Literal["image_url"]
    image_url: VisionImageURL


type VisionContent = VisionTextContent | VisionImageContent


_memory_image_cache: dict[str, tuple[bytes, float, GridOptions]] = {}
_CACHE_TTL_SECONDS = 120  # Safety TTL to prevent leaks if the consumer never fires


def store_temp_screenshot(raw_bytes: bytes, *, grid_options: Mapping[str, int | float] | None = None) -> str:
    ...
    _memory_image_cache[img_id] = (raw_bytes, time.monotonic(), dict(grid_options or {}))
...
def pop_temp_screenshot(img_id: str) -> tuple[bytes, GridOptions] | None:
...
_SCREENSHOT_PATH_RE = re.compile(r"workspace/(?:desktop[_-])?screenshot[_-]\d+\.png")
...
def _draw_coordinate_grid(
    ...
    *,
    ...
    minor_pixel_step = max(1, round(minor_step * pixel_scale))

    for x in range(0, width + 1, minor_pixel_step):
        absolute_x = round(origin_x + (x / pixel_scale))
    ...
    for y in range(0, height + 1, minor_pixel_step):
        absolute_y = round(origin_y + (y / pixel_scale))
    ...
    if origin_x or origin_y:
        desktop_width = round(width / pixel_scale)
        desktop_height = round(height / pixel_scale)
...
def compress_bytes_to_base64(
    ...
    *,
    ...
    grid_options: Mapping[str, int | float] | None = None,
) -> str | None:
    ...
    try:
        ...
        logger.info(f"[VisionInject] Compressed (Memory): {img.width}x{img.height}, {size_kb:.0f}KB{suffix}")
