...
class ConsumedEmailVerificationToken(TypedDict):
...
class EmailVerificationService:
    ...
    async def create_email_verification_token(self, identity_id: uuid.UUID, email: str) -> tuple[str, datetime]:
        ...
        token_key = f"{EMAIL_VERIFICATION_CODE_KEY_PREFIX}{code_hash}"
    ...
    async def consume_email_verification_token(self, raw_token: str) -> ConsumedEmailVerificationToken | None:
        """Load a valid verification code from Redis and mark it used (by deleting)."""
        import json

        redis = await get_redis()
        token_hash = self._hash_token(raw_token)
        token_key = f"{EMAIL_VERIFICATION_CODE_KEY_PREFIX}{token_hash}"
    ...
    async def send_verification_email(
        ...
        from app.services.system_email_service import render_email_template, send_system_email
