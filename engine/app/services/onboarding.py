    ...
    *,
    ...
    async with connection_ctx() as conn:
        existing = await conn.fetchone(
            "SELECT phase FROM agent_user_onboardings WHERE agent_id = %(agent_id)s AND user_id = %(user_id)s LIMIT 1",
            {"agent_id": agent.id, "user_id": user_id},
        )
        existing_phase = existing["phase"] if existing else None
        ...
        if agent.template_id:
            tpl = await conn.fetchone(
                "SELECT capability_bullets, bootstrap_content FROM agent_templates WHERE id = %(template_id)s",
    ...
    if existing_phase == PHASE_GREETED:
        ...
        if is_template_agent:
            ...
            prompt = (
                template_prompt.replace("{name}", agent.name)
...
async def mark_onboarding_phase(
    ...
    async with connection_ctx() as conn:
        await conn.execute(
            ...
            ON CONFLICT (agent_id, user_id) DO UPDATE SET phase = EXCLUDED.phase
            """,
            {"agent_id": agent_id, "user_id": user_id, "phase": phase},
        )
...
async def is_onboarded(
    ...
    async with connection_ctx() as conn:
        row = await conn.fetchone(
            "SELECT 1 FROM agent_user_onboardings WHERE agent_id = %(agent_id)s AND user_id = %(user_id)s LIMIT 1",
