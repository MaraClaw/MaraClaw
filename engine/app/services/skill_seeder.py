...
from app.dao import agent_dao, skill_dao, system_setting_dao
from app.db.session import connection_ctx


class BuiltinSkillFile(TypedDict):
    path: str
    content: str


class BuiltinSkill(TypedDict):
    name: str
    description: str
    category: str
    icon: str
...
## Asset Class Coverage (maraclaw roadmap)
...
maraclaw does **not** ship a dedicated calendar MCP server in v1. Smithery doesn't yet have a robust earnings/macro calendar tool. So this skill is a **structured wrapper around `web-research`** with curated query templates and source preferences. v2 will add a dedicated MCP backed by a free API (likely finnhub or trading-economics).
...
When maraclaw builds a dedicated finance-calendar MCP server, this skill will switch to direct API calls:
...
- If Vercel tools are enabled but the `vercel_token` is missing or empty, guide the user:
  ...
  3. Click "Create" to generate a new token (suggested name: "maraclaw", Scope: "Full Account").
  4. Copy the generated token, return to the MaraClaw tool settings page, and paste it into the "Vercel Access Token" configuration field for "Deploy to Vercel" or any other Vercel tools.
...
- If the project requires a database (Postgres), guide the user:
  ...
  3. Click "Create new API Key", name it (e.g., "maraclaw"), and copy the generated key.
  4. Return to the MaraClaw tool settings page, find the `Create Postgres Database` tool, and paste the key into the "Neon API Key" configuration field.
    ...
    async with connection_ctx():
        for skill_data in BUILTIN_SKILLS:
            ...
            if existing is None:
                ...
                logger.info(f"[SkillSeeder] Upserted skill: {skill_data['name']}")
...
async def push_default_skills_to_existing_agents():
    ...
    async with connection_ctx():
        default_skills = await skill_dao.list_defaults_with_files()
        ...
        setting = await system_setting_dao.get_by_key("default_skills_sync_hash")
        if setting and isinstance(setting.value, dict) and setting.value.get("hash") == current_hash:
            logger.info(
                f"[SkillSeeder] Default skills sync hash '{current_hash}' matches, skipping sync for existing agents"
            )
            return
        ...
        for agent_id in agent_ids:
            ...
            if await storage.is_file(legacy_key):
                try:
                    ...
                    logger.warning(
                        f"[SkillSeeder] Failed to remove legacy MCP_INSTALLER.md for agent {agent_id}: {exc}"
                    )
            for skill in default_skills:
                ...
                for sf in skill.files:
                    ...
                    pushed += 1
                logger.info(f"[SkillSeeder] Pushed new default skill '{skill.name}' to agent {agent_id}")
