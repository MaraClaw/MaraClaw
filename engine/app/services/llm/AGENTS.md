...
- ChatGPT subscription rows stay on provider `openai` with `auth_kind=chatgpt_subscription`. Tokens are Codex OAuth, not Platform API keys. Engine calls go through `create_llm_client_from_model` → Codex `/responses` with `chatgpt-account-id`.
