...
- New LLM provider/protocol work goes under `llm/`, preferably in `base.py`, `types.py`, `registry.py`, `factory.py`, or `providers/` rather than the compatibility-heavy `llm/client.py`. Company pool RBAC (who may add keys / assign tenant models) lives in `enterprise_llm.py`; routes stay thin. SuperGrok / X Premium device-code is `grok_oauth.py` (fakeable HTTP) + `grok_subscription.py` (session + encrypted pool upsert + refresh). ChatGPT Plus / Pro / Team device-code is `chatgpt_oauth.py` + `chatgpt_subscription.py` (same handoff; Codex JSON device-auth, not RFC 8628). Engine inference uses `create_llm_client_from_model` (Codex Responses), not `/chat/completions`.
- New document conversion belongs in `document_conversion/`, not in `agent_tools.py`.
- Inbound office-document parsing belongs in `document_parser/` (local `anydoc` / `firecrawl-anydoc`). Do not add format-specific extractors in `files.py` or `agent_tool_exec/document_reading.py`. `text_extractor.py` is gone — do not recreate it. Ordinary text decoding stays local. Do not route CSV/HTML/Markdown generation through anydoc.
