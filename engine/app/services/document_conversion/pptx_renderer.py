...
async def render_html_to_pptx(
    ...
    try:
        ...
        def string_attribute(value: str | AttributeValueList | None) -> str | None:
            if isinstance(value, str) or value is None:
                return value
            raise TypeError("HTML attribute must be a string")
        ...
        def element_style(el: Tag | None) -> dict[str, str]:
            ...
            style.update(parse_style(string_attribute(el.get("style"))))
            return style
        ...
        def css_px_to_inches(value: str | None, axis_px: int = 1280) -> float:
            ...
            try:
                if raw.endswith("px"):
                    return float(raw[:-2]) * (slide_width / 914400) / axis_px
                if raw.endswith("rem"):
                    return float(raw[:-3]) * 16 * (slide_width / 914400) / axis_px
                if raw.endswith("em"):
                    return float(raw[:-2]) * 16 * (slide_width / 914400) / axis_px
            except ValueError:
                return 0.0
            return 0.0
        ...
        def add_card(slide, x: float, y: float, w: float, h: float, style: dict[str, str]):
            ...
            border_width = str(style.get("border-width") or "0").replace("px", "").strip() or "0"
            if border_width.replace(".", "", 1).isdigit():
                has_border = has_border or float(border_width) > 0
            ...
            if not bg and not border:
                return None
            radius = length_to_inches(style.get("border-radius"), slide_width / 914400, design_w_px) or 0
        ...
        def element_box(style: dict[str, str]) -> tuple[float | None, float | None, float | None, float | None]:
            sw = slide_width / 914400
            sh = slide_height / 914400
            return (
        ...
        def render_flow_element(slide, el: Tag, y: float, x: float = 0.75, width: float = 11.85) -> float:
            ...
            if (style.get("position") == "absolute" or left is not None or top is not None) and (
                ...
                return y
            if name == "img":
                p = image_path(string_attribute(el.get("src")))
                if p:
                    h = box_h or 2.2
        ...
        if not slide_nodes:
            slide_nodes = list(body.find_all(["section", "article"], recursive=False))
        if not slide_nodes:
            slide_nodes = [body]
