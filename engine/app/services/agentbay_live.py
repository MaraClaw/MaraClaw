...
async def get_desktop_screenshot(agent_id: uuid.UUID, session_id: str = "") -> str | None:
    ...
    if cache_key not in _agentbay_sessions:
        ...
        for key in _agentbay_sessions:
            ...
            for key in _agentbay_sessions:
                ...
                logger.info(
                    f"[AgentBay_DEBUG] No computer session for agent {agent_id}. Keys available: {list(_agentbay_sessions.keys())}"
                )
                return None
...
async def get_browser_snapshot(agent_id: uuid.UUID, session_id: str = "") -> str | None:
    ...
    if cache_key not in _agentbay_sessions:
        for key in _agentbay_sessions:
            ...
            for key in _agentbay_sessions:
                ...
                logger.info(
                    f"[AgentBay_DEBUG] No browser session for agent {agent_id} (searchstr: {agent_id!s}). Keys available: {list(_agentbay_sessions.keys())}"
                )
                return None
...
def detect_agentbay_env(tool_name: str) -> str | None:
