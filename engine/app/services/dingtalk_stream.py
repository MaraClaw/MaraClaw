...
class _DingTalkText(TypedDict, total=False):
    content: str


class _DingTalkRichTextItem(TypedDict, total=False):
    text: str
    downloadCode: str


class _DingTalkContent(TypedDict, total=False):
    downloadCode: str
    richText: list[_DingTalkRichTextItem | list[_DingTalkRichTextItem]]
    recognition: str
    duration: str | int
    fileName: str


class _DingTalkMessage(TypedDict, total=False):
    msgtype: str
    text: _DingTalkText
    content: _DingTalkContent
    downloadCode: str
...
def _parse_rich_text_item(value: object) -> _DingTalkRichTextItem | None:
    item_data = _string_keyed_object(value)
    if item_data is None:
        return None

    item: _DingTalkRichTextItem = {}
    text = item_data.get("text")
    if isinstance(text, str):
        item["text"] = text
    download_code = item_data.get("downloadCode")
    if isinstance(download_code, str):
        item["downloadCode"] = download_code
    return item or None


def _parse_dingtalk_message(payload: object) -> _DingTalkMessage:
    payload_data = _string_keyed_object(payload)
    if payload_data is None:
        raise ValueError("DingTalk callback payload must be an object")

    message: _DingTalkMessage = {}
    msgtype = payload_data.get("msgtype")
    if isinstance(msgtype, str):
        message["msgtype"] = msgtype
    download_code = payload_data.get("downloadCode")
    if isinstance(download_code, str):
        message["downloadCode"] = download_code

    text_data = _string_keyed_object(payload_data.get("text"))
    if text_data is not None:
        text_content = text_data.get("content")
        if isinstance(text_content, str):
            message["text"] = {"content": text_content}

    content_data = _string_keyed_object(payload_data.get("content"))
    if content_data is not None:
        content: _DingTalkContent = {}
        for key in ("downloadCode", "recognition", "fileName"):
...
async def _get_media_download_url(access_token: str, download_code: str, robot_code: str) -> str | None:
...
async def download_dingtalk_media(app_key: str, app_secret: str, download_code: str) -> bytes | None:
        ...
        for section in rich_text:
            for item in section if isinstance(section, list) else [section]:
                if "text" in item:
                    ...
                    file_bytes = await download_dingtalk_media(app_key, app_secret, item["downloadCode"])
                    if file_bytes:
    ...
    if msgtype == "file":
        ...
        if download_code:
            ...
            if file_bytes:
                ...
                logger.info(
                    f"[DingTalk] Saved file '{original_filename}' to {workspace_path} ({len(file_bytes)} bytes)"
                )
                return (
...
async def _upload_dingtalk_media(
    ...
    if not await asyncio.to_thread(file_p.exists):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            upload_url = f"https://oapi.dingtalk.com/media/upload?access_token={access_token}&type={media_type}"
            resp = await client.post(
                upload_url,
...
async def _send_dingtalk_media_message(
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            logger.info(f"[DingTalk] Sent {media_type} message to {target_id[:16]}... (conv_type={conversation_type})")
            return True
    except Exception as e:
...
class DingTalkStreamManager:
    ...
    def _run_client_thread(
        ...
        while not stop_event.is_set() and retries <= max_retries:
            try:
                ...
                logger.info(
                    f"[DingTalk Stream] Connecting for agent {agent_id}... (attempt {retries + 1}/{max_retries + 1})"
                )
                ...
                retries = 0
                retries += 1
                logger.warning(f"[DingTalk Stream] Connection lost for agent {agent_id}, will retry...")

            except Exception as e:
                retries += 1
                logger.error(
                    f"[DingTalk Stream] Connection error for {agent_id} (attempt {retries}/{max_retries + 1}): {e}"
                )

            if retries > max_retries:
                logger.error(f"[DingTalk Stream] Agent {agent_id} exhausted all {max_retries} retries, giving up")
                break

            delay = retry_delays[min(retries - 1, len(retry_delays) - 1)]
            logger.info(f"[DingTalk Stream] Retrying in {delay}s for agent {agent_id}...")
    ...
    async def start_all(self):
        ...
        configs = await channel_config_dao.list_configured("dingtalk")
