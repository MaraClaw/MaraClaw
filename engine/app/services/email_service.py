...
class EmailProviderPreset(TypedDict):
    label: str
    imap_host: str
    imap_port: int
    smtp_host: str
...
class EmailConfig(TypedDict):
    email_provider: NotRequired[str]
    email_address: NotRequired[str]
    auth_code: NotRequired[str]
    imap_host: NotRequired[str]
    imap_port: NotRequired[int | str]
    smtp_host: NotRequired[str]
    smtp_port: NotRequired[int | str]
    smtp_ssl: NotRequired[bool]


class ResolvedEmailConfig(TypedDict):
    email_address: str
    auth_code: str
    imap_host: str
    imap_port: int
...
class EmailConnectionResult(TypedDict):
    ok: bool
    imap: NotRequired[str]
    smtp: NotRequired[str]
    error: NotRequired[str]


# Preset email provider configurations
EMAIL_PROVIDERS: dict[str, EmailProviderPreset] = {
    "qq": {
        "label": "QQ Mail",
    ...
    if attachments and workspace_path:
        ...
        for rel_path in attachments:
            ...
            try:
                ...
                logger.warning(f"Could not load email attachment from storage {clean_rel}: {storage_error}")

            # 2. Fall back to local disk if not found in storage backend
            if file_bytes is None:
                full_path = workspace_path / rel_path
                try:
                    async with aiofiles.open(full_path, "rb") as attachment_file:
                        file_bytes = await attachment_file.read()
                    filename = full_path.name
                except OSError as local_error:
                    logger.warning(f"Could not load local email attachment {full_path}: {local_error}")
...
async def test_connection(config: EmailConfig) -> EmailConnectionResult:
    ...
    result: EmailConnectionResult = {"ok": True, "imap": "", "smtp": ""}
    ...
    try:
        send_smtp_email(
            ...
            msg_string=f"From: {addr}\nTo: {addr}\nSubject: MaraClaw Connection Test\n\nSMTP Connection Successful.",
            use_ssl=cfg.get("smtp_ssl", True),
            timeout=10,
        )
