...
def _calendar_support() -> ModuleType:
...
async def _get_agent_calendar_id(token: str) -> tuple[str | None, str | None]:
    ...
    if code == 0:
        ...
        if calendars:
            return calendars[0].get("calendar", {}).get("calendar_id"), None
...
async def _feishu_calendar_list(agent_id: uuid.UUID, arguments: ToolArguments) -> str:
    ...
    start_ts = support._to_unix(start_arg, now)
    end_ts = support._to_unix(end_arg, now + timedelta(days=7))
    start_iso = support._to_iso(start_arg, now)
    end_iso = support._to_iso(end_arg, now + timedelta(days=7))
    ...
    params: dict[str, ToolArgumentValue] = {}
    if start_ts:
    ...
    if not items and not freebusy_section:
        return "📅 该时间段内没有日程。"
    lines = _calendar_support()._format_calendar_items(items)
    if freebusy_section:
...
async def _freebusy_section(sender_open_id: str, token: str, start_iso: str, end_iso: str) -> str:
    try:
        ...
        if data.get("code") != 0:
            return ""
        busy_slots = data.get("data", {}).get("freebusy_list", [])
        if not busy_slots:
            return "\n📌 **用户真实日历**：该时段全部空闲。"
        ...
        for slot in sorted(busy_slots, key=lambda item: item.get("start_time", "")):
            try:
                start = datetime.fromisoformat(slot["start_time"]).astimezone(tz_cn).strftime("%H:%M")
                end = datetime.fromisoformat(slot["end_time"]).astimezone(tz_cn).strftime("%H:%M")
                busy_lines.append(f"  🔴 {start}–{end}")  # noqa: RUF001
            except Exception:
                busy_lines.append(f"  🔴 {slot.get('start_time')}–{slot.get('end_time')}")  # noqa: RUF001
        return "\n📌 **用户真实日历（忙碌时段）**：\n" + "\n".join(busy_lines)
...
async def _feishu_calendar_create(agent_id: uuid.UUID, arguments: ToolArguments) -> str:
    ...
    attendee_open_ids, attendee_display = await _calendar_support()._calendar_attendees(
        agent_id, arguments, token, user_email
    )
...
async def _feishu_calendar_update(agent_id: uuid.UUID, arguments: ToolArguments) -> str:
    ...
    agent_tools, token, agent_cal_id, error = await _calendar_support()._calendar_write_context(agent_id, user_email)
    if error:
        return error
...
async def _feishu_calendar_delete(agent_id: uuid.UUID, arguments: ToolArguments) -> str:
    ...
    if not user_email or not event_id:
        return "❌ Both 'user_email' and 'event_id' are required."
    _facade, token, agent_cal_id, error = await _calendar_support()._calendar_write_context(agent_id, user_email)
    if error:
        return error
    async with _httpx_module().AsyncClient(timeout=20) as client:
