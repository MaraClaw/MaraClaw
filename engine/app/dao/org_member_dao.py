...
class OrgMemberDAO(BaseDAO[OrgMemberRecord]):
    ...
    async def find_unbound_by_email(self, email: str, tenant_id: Any) -> OrgMemberRecord | None:
    ...
    async def find_unbound_by_phone(self, phone: str, tenant_id: Any) -> OrgMemberRecord | None:
        async with self.session() as db:
            row = await db.fetchone(
                f"SELECT {self._select_list()} FROM org_members "
    ...
    async def update_fields(self, member_id: Any, fields: Mapping[str, Any]) -> OrgMemberRecord | None:
    ...
    async def get_by_unionid(self, provider_id: Any, unionid: str) -> OrgMemberRecord | None:
        async with self.session() as db:
            row = await db.fetchone(
                f"SELECT {self._select_list()} FROM org_members "
    ...
    async def get_by_provider_and_open_id(self, provider_id: Any, open_id: str) -> OrgMemberRecord | None:
        async with self.session() as db:
            row = await db.fetchone(
                f"SELECT {self._select_list()} FROM org_members "
    ...
    async def get_by_provider_and_external_id(self, provider_id: Any, external_id: str) -> OrgMemberRecord | None:
        async with self.session() as db:
            row = await db.fetchone(
                f"SELECT {self._select_list()} FROM org_members "
    ...
    async def find_related_to_agent_by_name(self, agent_id: Any, name: str) -> OrgMemberRecord | None:
    ...
    async def list_active_with_user_for_tenant(self, tenant_id: Any) -> Sequence[OrgMemberRecord]:
        async with self.session() as db:
            rows = await db.fetchall(
                f"SELECT {self._select_list()} FROM org_members "
    ...
    async def names_for_ids(self, member_ids: Sequence[Any]) -> dict[Any, str]:
        if not member_ids:
            return {}
        async with self.session() as db:
            rows = await db.fetchall(
    ...
    async def get_user_id(self, member_id: Any) -> Any | None:
        async with self.session() as db:
    ...
    async def list_relationship_candidates(
        ...
        *,
        tenant_id: Any,
        search: str | None = None,
        allowed_user_ids: Sequence[Any] | None = None,
        limit: int = 200,
    ) -> list[tuple[OrgMemberRecord, str | None, str | None, Any | None]]:
