...
class TenantEmailDomainDAO(BaseDAO[TenantEmailDomainRecord]):
    ...
    async def list_for_tenant(self, tenant_id: Any) -> Sequence[TenantEmailDomainRecord]:
        async with self.session() as db:
    ...
    async def get_default_for_tenant(self, tenant_id: Any) -> TenantEmailDomainRecord | None:
        async with self.session() as db:
            row = await db.fetchone(
                f"SELECT {self._select_list()} FROM tenant_email_domains "
    ...
    async def clear_default_for_tenant(self, tenant_id: Any) -> None:
        async with self.session() as db:
            await db.execute(
