...
class AgentRelationshipDAO(BaseDAO[AgentRelationshipRecord]):
    ...
    async def get_active_for_agent_by_feishu_id(self, agent_id: UUID, feishu_id: str) -> SimpleNamespace | None:
    ...
    async def list_for_agent_with_members_and_providers(self, agent_id: UUID) -> list[SimpleNamespace]:
...
def _relationship_with_member(row: dict[str, Any], *, include_provider: bool = False) -> SimpleNamespace:
    member = None
    ...
    if include_provider:
        result.provider_name = row.get("provider_name")
        result.provider_type = row.get("provider_type")
    return result
