...
def _log_bwrap_startup_status() -> None:
    """Emit a startup diagnostic for bubblewrap availability and isolation capability.

    We only warn when bwrap is missing or unusable so deployments can still start.
    Local source runs may explicitly allow a reduced-isolation fallback, while
...
async def _start_ss_local() -> None:
    ...
    for node in nodes:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as temp_file:
            json.dump(cfg, temp_file)
        try:
            proc = await asyncio.create_subprocess_exec(
                "ss-local", "-c", temp_file.name, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.PIPE
            )
            ...
            stderr = proc.stderr
            if stderr is None:
                logger.warning(f"[Proxy] {node['label']} failed without error output")
                continue
            err = (await stderr.read()).decode()[:120]
...
async def lifespan(app: FastAPI):
    ...
    def _bg_task_error[TaskResult](task: asyncio.Task[TaskResult]) -> None:
        ...
        try:
            ...
            return
        if exc:
            logger.error(f"[startup] Background task {task.get_name()} CRASHED: {exc}")
            import traceback
    ...
    if _role_enabled("all", "bootstrap"):
        # Schema is applied by app.scripts.bootstrap_db (scripts/schema_baseline.sql).
        #
        ...
        try:
            ...
            if _old_dir.exists() and any(_old_dir.iterdir()):
                _tenant = await tenant_dao.get_first_by_created_at()
                if _tenant:
    ...
    try:
        ...
        logger.warning(f"[shutdown] logging shutdown failed: {e}")
    try:
...
def _load_version_info() -> dict[str, str]:
    ...
    if not commit:
        ...
        if git_path:
            try:
                commit = (
                    subprocess.check_output(  # noqa: S603 - git_path comes only from shutil.which; argv is fixed.
...
async def get_version():
    """Return current MaraClaw version and commit hash."""
