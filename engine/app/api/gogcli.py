...
async def _get_gogcli_manage_agent(db, current_user: UserRecord, agent_id: uuid.UUID):
...
def _require_gogcli_container(agent) -> None:
...
async def set_gogcli_keyring_secret(
    ...
    await run_sync(write_gogcli_keyring_secret, agent.id, password)
    await upsert_gogcli_keyring_password(db, agent.id, password)
...
async def get_gogcli_auth_status(
    ...
    if result.authenticated:
        await capture_authenticated_gogcli_state(db, agent.id, agent_manager._agent_dir(agent.id), result)
