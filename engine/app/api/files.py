...
async def read_file(agent_id: uuid.UUID, path: str, current_user: UserRecord = Depends(get_current_user)):
    ...
    try:
        ...
        return FileContent(
            path=path,
            content=f"[Binary file: {Path(path).name}, {stat.size} bytes]",
...
def _detect_csv_delimiter(text: str) -> str:
    ...
    for candidate in candidates:
        candidate_score = sum(line.count(candidate) for line in lines)
...
async def preview_file(
    ...
    companion = _find_companion_text_preview(local_target) if local_target is not None else None
    if companion is not None:
        ...
        return {
            ...
            "content_hash": content_hash(content or ""),
            "companion_path": str(companion.resolve().relative_to(_agent_base_dir(agent_id).resolve()))
            if not path.startswith("enterprise_info")
            else None,
            "download_url": download_url,
        }
...
async def import_skill_to_agent(
    ...
    skill_files = await skill_dao.list_files(skill.id)
    if not skill_files:
...
async def upload_file_to_workspace(
    ...
    if normalized_path not in {"workspace", "skills"} and not normalized_path.startswith(("workspace/", "skills/")):
        raise HTTPException(
            status_code=400,
            detail="The right-side root view is the agent root directory; upload files to workspace/ or skills/.",
        )
...
async def read_enterprise_file(
    ...
    try:
        ...
        return {"path": path, "content": f"[Binary file: {Path(path).name}, {stat.size} bytes]"}
