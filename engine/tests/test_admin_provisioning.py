...
async def test_set_end_user_active_org_admin_toggles_member(monkeypatch):
    ...
    agent = SimpleNamespace(id=uuid.uuid4(), container_id="abc123", name="Daisy")
    monkeypatch.setattr(
        provisioning.user_dao,
        "update",
        AsyncMock(return_value=_user(role="member", tenant_id=tenant_id, is_active=False, user_id=target.id)),
    )
    disable_agents = AsyncMock(return_value=[agent])
    disable_triggers = AsyncMock(return_value=1)
    disable_schedules = AsyncMock(return_value=1)
    stop_container = AsyncMock(return_value=True)
    monkeypatch.setattr(provisioning.agent_dao, "disable_for_creator", disable_agents)
    monkeypatch.setattr(provisioning.agent_trigger_dao, "disable_for_creator", disable_triggers)
    monkeypatch.setattr(provisioning.agent_schedule_dao, "disable_for_creator", disable_schedules)
    monkeypatch.setattr("app.services.agent_manager.agent_manager.stop_container", stop_container)
    ...
    assert updated.is_active is False
    disable_agents.assert_awaited_once_with(target.id, tenant_id=tenant_id)
    disable_triggers.assert_awaited_once_with(target.id, tenant_id=tenant_id)
    disable_schedules.assert_awaited_once_with(target.id, tenant_id=tenant_id)
    stop_container.assert_awaited_once_with(agent)


@pytest.mark.asyncio
async def test_set_end_user_active_platform_admin_cross_tenant(monkeypatch):
    actor = _user(role="platform_admin")
    target = _user(role="agent_admin", tenant_id=uuid.uuid4(), is_active=True)
    monkeypatch.setattr(
        provisioning.user_dao,
        "update",
    ...
    monkeypatch.setattr(provisioning.agent_schedule_dao, "disable_for_creator", AsyncMock(return_value=0))
    with patch("app.services.admin_provisioning.connection_ctx") as ctx:
        ctx.return_value.__aenter__ = AsyncMock()
        ctx.return_value.__aexit__ = AsyncMock(return_value=False)
        with patch("app.services.admin_provisioning.write_admin_audit", AsyncMock()):
...
@pytest.mark.asyncio
async def test_set_end_user_active_is_noop_when_already_set():
    tenant_id = uuid.uuid4()
    actor = _user(role="org_admin", tenant_id=tenant_id)
    target = _user(role="member", tenant_id=tenant_id, is_active=False)
    updated = await provisioning.set_end_user_active(actor=actor, target=target, is_active=False)
