...
def test_find_finish_call_validates_arguments():
    ...
    assert missing_content.error is not None
...
async def test_call_llm_requires_finish_tool_to_stop(monkeypatch):
    ...
    monkeypatch.setattr(caller, "_get_agent_config", lambda *_args, **_kwargs: _async_return((3, None)))
    ...
    monkeypatch.setattr(caller, "create_llm_client_from_model", lambda *_a, **_k: fake_client)
    ...
    assert any(msg.role == "user" and msg.content for msg in second_round_messages)
...
async def test_invalid_finish_does_not_stop_and_is_returned_as_tool_error(monkeypatch):
    ...
    second_round_messages = fake_client.messages_seen[1]
    assert any(
        msg.role == "tool" and msg.tool_call_id == "call_finish" and "content" in str(msg.content)
...
async def test_skip_tools_still_exposes_finish(monkeypatch):
    ...
    monkeypatch.setattr(caller, "_get_agent_config", lambda *_args, **_kwargs: _async_return((1, None)))
    monkeypatch.setattr(caller, "_get_user_name", lambda _user_id: _async_return("Ray"))
    monkeypatch.setattr(
        "app.services.agent_context.build_agent_context",
        lambda *_args, **_kwargs: _async_return(("static", "dynamic")),
...
def test_finish_is_in_always_available_core_tools():
    from app.services.agent_tools_definitions import _ALWAYS_INCLUDE_CORE
...
async def test_mid_loop_token_limit_checking(monkeypatch):
    ...
    async def mock_get_agent_config(*_args, **_kwargs):
