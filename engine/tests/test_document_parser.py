...
from app.services.document_parser.errors import DocumentParseTimeoutError, DocumentTooLargeError
from app.services.document_parser.runtime import convert_worker
...
def test_pyproject_pins_published_firecrawl_anydoc() -> None:
    ...
    assert "FROM python:3.14.7-slim-trixie AS production" in dockerfile.read_text(encoding="utf-8")
