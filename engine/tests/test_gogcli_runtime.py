...
def test_gog_sheets_skill_documents_core_commands() -> None:
    ...
    assert "| `datasource` | Manage Connected Sheets data sources and extracts |" in skill_text
...
def test_gog_gmail_and_drive_skills_document_v0350_commands() -> None:
    ...
    # Then - commands added in the gogcli 0.34.x/0.39.0 skill surface
    ...
    assert "`--results-only` unwraps the primary result before `--select` projects it." in gog_text
...
def test_gog_adsense_and_slides_skills_document_v0390_commands() -> None:
    # Given
    root = Path(__file__).resolve().parents[1] / "app/services/gogcli_skill_files"
    adsense_text = (root / "gog-adsense" / "SKILL.md").read_text(encoding="utf-8")
    slides_text = (root / "gog-slides" / "SKILL.md").read_text(encoding="utf-8")
    appscript_text = (root / "gog-appscript" / "SKILL.md").read_text(encoding="utf-8")

    # Then - commands added in the gogcli 0.39.0 skill surface
    assert "<!-- Generated by scripts/gen-agent-skills.mjs; do not edit. -->" in adsense_text
    assert "| `reports` | Generate AdSense reports |" in adsense_text
    assert "| `skip-slide` | Skip a slide during presentation |" in slides_text
    assert "| `unskip-slide` | Include a skipped slide during presentation |" in slides_text
    assert "| `deployments` | List deployments |" in appscript_text
    assert "| `pull` | Pull an Apps Script project into a local directory |" in appscript_text
